defmodule ElixirGist.GistsTest do
  ...
  describe "saved_gists" do
    ...
    test "list_saved_gists/0 returns all saved_gists" do
      save_gist = save_gist_fixture()
      assert Gists.list_saved_gists() == [save_gist]
    end

    test "get_save_gist!/1 returns the save_gist with given id" do
      save_gist = save_gist_fixture()
      assert Gists.get_save_gist!(save_gist.id) == save_gist
    end

    test "create_save_gist/1 with valid data creates a save_gist" do
      valid_attrs = %{}

      assert {:ok, %SaveGist{} = save_gist} = Gists.create_save_gist(valid_attrs)
    end

    test "create_save_gist/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Gists.create_save_gist(@invalid_attrs)
    end

    test "update_save_gist/2 with valid data updates the save_gist" do
      save_gist = save_gist_fixture()
      update_attrs = %{}

      assert {:ok, %SaveGist{} = save_gist} = Gists.update_save_gist(save_gist, update_attrs)
    end

    test "update_save_gist/2 with invalid data returns error changeset" do
      save_gist = save_gist_fixture()
      assert {:error, %Ecto.Changeset{}} = Gists.update_save_gist(save_gist, @invalid_attrs)
      assert save_gist == Gists.get_save_gist!(save_gist.id)
    end

    test "delete_save_gist/1 deletes the save_gist" do
      save_gist = save_gist_fixture()
      assert {:ok, %SaveGist{}} = Gists.delete_save_gist(save_gist)
      assert_raise Ecto.NoResultsError, fn -> Gists.get_save_gist!(save_gist.id) end
    end

    test "change_save_gist/1 returns a save_gist changeset" do
      save_gist = save_gist_fixture()
      assert %Ecto.Changeset{} = Gists.change_save_gist(save_gist)
    end
  end
end
